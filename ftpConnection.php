...
class FTPConnection{
    ...
    private function handleSemaphore(){
        ...
        if(in_array($this->ftp_folder_path."/".$this->semaphore_name, $remote_contents) ){
            $this->logger->postMessage("Ops...another process seems to be in place. Please, retry!", "WARNING");
            return;
        }else{ 
            ...
            if( !ftp_put($this->connection, $this->semaphore_name, $this->semaphore_name, FTP_ASCII) ){
                throw new HandleOperationsException("Error uploading semaphore to remote directory.");
            }
    ...
    public function __destruct(){
        if(!is_null($this->connection)){
            if( !ftp_close($this->connection) )
                throw new HandleOperationsException("Unable to close connection.");
        }
    }
