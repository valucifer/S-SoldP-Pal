	...
	class Mapping{
		...
		private function getArtAndAttribute(){
			...
			for($i = 0; $i < $sizeArrayTBART; $i++){
				...
				if(sizeof($tmp) === 1) 
					break;
				$width = "[Larghezza]:".$tmp[12];
				$height = ",[Altezza]:".$tmp[13];
				$size = ",[Lunghezza]:".$tmp[11];
				
				$return[$tmp[0]] = $width.$height.$size;
			}
		...
		public function getItemMaster(){
			...
			foreach($this->keys as $keys){
				...
				$stringTmp = "";
				$count = 0; //Serve unicamente a non stampare la ',' all'ultima foto.
				foreach($urlFoto[$keys] as $singleFoto){
					$count++;
					if($count != sizeof($urlFoto[$keys]))
